...
#library(readr)
#library(dplyr)
#df<- read_csv <- ("C:/Users/USER/Desktop/tmdb-movies.csv")
#df <- read_csv(path)
#View(df)
...
print(na_val
      ...
      #b) ordered using revenue in descending order
      
      ordered_rev <- d[order(d$sum_revenue, decreasing = TRUE), ]  
      View(ordered_rev)
      
      #c) limited by ten rows and order by budget in ascending order
